...
class World:
  ...
  def setup(self, tmx_map, overworld_frmes):
    ...
    for col in range(tmx_map.width):
      for row in range(tmx_map.height):
        AnimatedSprite((col * TILE_SIZE, row * TILE_SIZE), overworld_frmes['water'], self.all_sprites, Z_LAYERS['bg']) 
    
    
    for obj in tmx_map.get_layer_by_name('Objects'):
      if obj.name == 'palm':
        AnimatedSprite((obj.x, obj.y), overworld_frmes['palm'] , self.all_sprites, Z_LAYERS['main'], randint(4, 6))
      else:
    ...
    for obj in tmx_map.get_layer_by_name('Nodes'):
      ...
      if obj.name == 'Node' and obj.properties['stage'] == self.data.current_level:
        self.icon = Icon((obj.x + TILE_SIZE / 2, obj.y + TILE_SIZE / 2), self.all_sprites, overworld_frmes['icon'])
      
      
      if obj.name == 'Node':
        Node(
          pos=(obj.x, obj.y), 
          surf=overworld_frmes['path']['node'], 
