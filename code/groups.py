...
class AllSprites(pygame.sprite.Group):
  def __init__(self, width, height, clouds, horizon , bg_tile=None, top_limit=0):
    super().__init__()
    ...
    if bg_tile:
      for col in range(width):
        for row in range(-int(top_limit / TILE_SIZE) - 1, height):
          ...
          Sprite((x,y), bg_tile, self, Z_LAYERS['bg tiles'])
    else:
  ...
  def draw_large_cloud(self, dt):
    ...
    for cloud in range(self.large_cloud_tiles):
      x = self.large_cloud_x + self.large_cloud_width * cloud + self.offset.x
      y = self.horizon_line - self.large_cloud_height + self.offset.y
      self.display_surface.blit(self.large_cloud, (x,y))
